...
class VisionTransformer(tf.keras.Model):
    def __init__(self, image_size, patch_size, num_layers, num_classes, d_model, num_heads, mlp_dim, channels, dropout):
        ...
        self.mlp_dense2 = Dense(num_classes)
    ...
    def get_grim_top_right(self, matrix):
        ...
        #         matrix_1 = np.array(tf.reshape(matrix,[batch_size,-1,matrix.shape[3]]))

        features = []
        for index in range(batch_size):
            gram_matrix = np.dot(tf.transpose(matrix[index],[]), matrix[index])
            feature = []
            ...
            for row in range(gram_len):
                for clo in range(gram_len):
                    ...
                    if (clos > gram_len - 1):
                        break
                    feature.append((gram_matrix[row][row + clo]))
